...
rule integrityOfTransferFrom(address owner, address recipient, uint256 amount) {
	...
	// why is that necessary? try commenting this line out and see what happens
...
rule totalSupplyNotLessThanSingleUserBalance(method f, address user) {
	// @note after burn total supply will reduce, and after transfer and transferFrom, total supply will be same
	// this rule always checks for the balance to increase and that's why it will fail for even fixed version.
	env e;
	calldataarg args;
