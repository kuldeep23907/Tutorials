...
contract TicketDepot {
    ...
    function getRemainingTicket(uint16 eventId) public returns (uint16) {
        return eventsMap[eventId].ticketsRemaining;
    }

    function getTicketPrice(uint16 eventId) public returns (uint64) {
        return eventsMap[eventId].ticketPrice;
    }

    function getEventOwner(uint16 eventId) public returns (address) {
        return eventsMap[eventId].owner;
    }

    function getTotalNumberOfEvents(uint16 eventId) public returns (uint16) {
        return numEvents;
    }
}
